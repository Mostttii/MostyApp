...
async function getRecentRecipeUrls(count: number = 30): Promise<string[]> {
...
async function saveToCSV(recipes: Recipe[]) {
  const csvData = recipes.map(recipe => ({
    ...
    'Dietary Info': Object.entries(recipe.dietaryInfo)
