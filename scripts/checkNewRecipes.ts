...
interface LastParsed {
  timestamp: Date;
  url: string;
}

async function getLastParsed(): Promise<LastParsed | null> {
  try {
...
async function getNewRecipeUrls(): Promise<string[]> {
  try {
    ...
    // Look for href attributes containing "/recipe/" but not containing "#" (to avoid anchors)
...
async function parseAndStoreRecipe(url: string, parser: AllRecipesParser): Promise<Recipe | null> {
  try {
...
async function updateLastParsed(url: string) {
